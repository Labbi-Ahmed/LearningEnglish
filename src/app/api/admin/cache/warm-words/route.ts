...
export async function POST() {
  try {
    ...
    try {
      ...
      await writeLastRun("words", user.id, result);
      const entry = await readLastRun("words");
      ...
      console.error("[admin:warm-words] failed", err);
    ...
    console.error("[admin:warm-words] unexpected", error);
